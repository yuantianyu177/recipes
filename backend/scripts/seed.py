...
INGREDIENTS = [
    ...
    ("馄饨皮", "张", 10.0, "主料"),
    ("糯米", "克", 3.48, "主料"),
    # === Main ingredients - baking ===
    ("抹茶粉", "克", 3.24, "主料"), ("马斯卡彭", "克", 4.29, "主料"),
    ("手指饼干", "根", 20.0, "主料"),
    # === Side ingredients ===
...
async def seed_data():
    ...
    TAG_CAT_COLORS = [
        "#c45d3e", "#5b7a5e", "#b8860b", "#6b5b95", "#2e86ab",
        "#d4726a", "#3d7068", "#a0522d", "#708090", "#8b6914",
        "#c44569", "#3c6382",
    ]
    ING_CAT_COLORS = [
        "#2e86ab", "#a0522d", "#5b7a5e", "#6b5b95", "#d4726a",
        "#b8860b", "#3d7068", "#c45d3e", "#708090", "#8b6914",
        "#c44569", "#3c6382",
    ]
    ...
    async with session_factory() as db:
        # ===== Tag Categories =====
        tag_cats = {}
        for idx, name in enumerate(TAG_CATEGORIES):
            db.add(TagCategory(name=name, color=TAG_CAT_COLORS[idx % len(TAG_CAT_COLORS)]))
        await db.flush()
        ...
        ing_cats = {}
        for idx, name in enumerate(INGREDIENT_CATEGORIES):
            db.add(IngredientCategory(name=name, color=ING_CAT_COLORS[idx % len(ING_CAT_COLORS)]))
        await db.flush()
